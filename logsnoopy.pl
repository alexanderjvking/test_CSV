...
while(1)
{
    system("cat /var/log/ttylog/ttylog.\$\(whoami\).\$\(hostname\).* > /var/log/ttylog/alltty.\$\(hostname\)");
    system("python /usr/local/src/analyze.py /var/log/auth.log /var/log/ttylog/alltty.\$\(hostname\)" . " /var/log/ttylog/cli.\$\(hostname\).csv");
